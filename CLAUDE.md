...
### [MANDATORY] 커밋 메시지 작성 규칙

> **모든 커밋 메시지에는 빌드 검증 결과를 반드시 포함한다. 결과가 FAIL이어도 괜찮다. 누락이 금지다.**

**간결성 원칙**: 제목 한 줄 + 빌드 검증 상태만 작성한다. 파일 목록, 구현 상세, 중복 설명은 적지 않는다. `git diff`로 확인 가능한 내용은 커밋 메시지에 넣지 않는다.
...
[빌드 검증 상태]
- app-build: PASS / FAIL (핵심 이유만)
- unit-test: PASS / FAIL (핵심 이유만)
```
...
| 커밋 메시지에 빌드 검증 결과 미기재 | 이력만으로 당시 상태를 추적할 수 없게 됨 |
| 커밋 메시지에 파일 목록·구현 상세 기재 | `git diff`로 확인 가능한 내용은 중복이다 |
