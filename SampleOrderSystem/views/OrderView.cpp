...
void OrderView::showCreate() {
    std::string sampleId = ConsoleHelper::readLine("  시료 ID: ");
    std::string customer = ConsoleHelper::readLine("  고객사: ");
    int qty = ConsoleHelper::readInt("  주문 수량: ");
...
void OrderView::showApprove() {
    showReserved();
    std::string id = ConsoleHelper::readLine("  주문 ID: ");
    if (id.empty()) return;
    int action = ConsoleHelper::readInt("  1.승인  2.거절: ");
    bool ok = false;
